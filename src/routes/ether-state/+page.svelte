<script lang="ts">
	...
	import { balanceOnBlock } from '$lib/eth/stores/state'
...
<main class="flex flex-col p-10 w-screen h-screen items-center gap-5 font-serif">
	...
	<section class="flex flex-col p-10 items-start gap-5 border-2 rounded-xl">
		...
		<p class="text-md text-black font-semibold">
			Set In '$lib/eth/globalState.ts', will load as long as a store is used from '$lib/eth/stores/state.ts'
			or you import the globalState in the root of your project with 'import "$lib/eth/globalState";'
		</p>
		<h2 class="text-xl text-black font-semibold">
